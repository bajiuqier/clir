...
from data import DatasetForHIKE, DataCollatorForHIKE
...
def main():
    ...
    writer = SummaryWriter(os.path.join(tensorboard_logs_path, 'baseline_HIKE'))

    # 按日期命名日志文件
    current_date = datetime.now().strftime("%Y-%m-%d")
    log_file = os.path.join(logging_args.log_dir, f"baseline_HIKE_training_{current_date}.log")
    logging.basicConfig(
        level=logging.INFO,
