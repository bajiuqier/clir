...
def multithreading_fetch_item_info(qid_list: list, processed_qids: list) -> Tuple[pd.DataFrame, list]:
    results = []
    error_occurred = False  # 用于记录是否发生错误
...
if __name__ == "__main__":
    ...
    ADJ_ITEM_INFO_HOME_DIR = Path(__file__).parent.parent / 'base_adj_item_info'

    triplet_id_file = str(HOME_DIR / 'base_test11_triplet_id_filtered.csv')
    ...
    if isinstance(adj_item_qid_list[0], str):
        ...
        while len(processed_qids) < len(adj_item_qid_list):
            ...
            property_info_df, processed_qids = multithreading_fetch_item_info(qid_list=adj_item_qid_list,
                                                                              processed_qids=processed_qids)

            adj_item_info_file = str(ADJ_ITEM_INFO_HOME_DIR / f"base_adj_item_info_{loop_num}.csv")
